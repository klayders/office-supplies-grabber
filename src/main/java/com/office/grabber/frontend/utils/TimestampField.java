...
public class TimestampField extends AbstractNumberField<TimestampField, Byte> {

  private static final SerializableFunction<String, Byte> PARSER = valueFormClient -> {
    if (valueFormClient == null || valueFormClient.isEmpty()) {
      return null;
    }
    try {
      return Byte.parseByte(valueFormClient);
    } catch (NumberFormatException e) {
      return null;
    }
  };

  private static final SerializableFunction<Byte, String> FORMATTER = valueFromModel -> valueFromModel == null
      ? ""
  ...
  public TimestampField() {
    super(PARSER, FORMATTER, Byte.MIN_VALUE, Byte.MAX_VALUE);
  }
   ...
   */
  public TimestampField(String label, String placeholder) {
    this(label);
   ...
   */
  public TimestampField(
      ValueChangeListener<? super ComponentValueChangeEvent<TimestampField, Byte>> listener) {
    this();
   ...
   * @see #addValueChangeListener(ValueChangeListener)
   */
  public TimestampField(String label, Byte initialValue,
                        ValueChangeListener<? super ComponentValueChangeEvent<TimestampField, Byte>> listener) {
    this(label);
